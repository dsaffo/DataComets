      ...
      for (let i = 0; i < logs.length; i++) {
        ...
        if (log != '') {
          ...
          for (let i = 0; i < records.length; i++) {
            ...
            if (record == 'q[0]') {
              index = record.substring(record.length - 2, record.length - 1);
              for (let i = 0; i < log_data.length; i++) {
                let xyz = quatToEuler([data[log][i]['q[0]'], data[log][i]['q[1]'], data[log][i]['q[2]'], data[log][i]['q[3]']])

                xyz.forEach(function (e) {
                  xyz[xyz.indexOf(e)] = e * 180 / Math.PI;
                });
                //console.log([data[log][i]['q[0]'], data[log][i]['q[1]'], data[log][i]['q[2]'], data[log][i]['q[3]']])
                console.log(xyz)
                x.push(xyz[0])
                y.push(xyz[1])
                z.push(xyz[2])
              ...
              if (record == 'q[0]') {
                for (let i = 0; i < x.length; i++) {
                  log_data[i][record] = x[i]
                  title = 'roll'
                }
              } else if (record == 'q[1]') {
                for (let i = 0; i < y.length; i++) {
                  log_data[i][record] = y[i]
                  title = 'pitch'
