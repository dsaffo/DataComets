const Tree = (function (dispatch, data, dimensions) {
  ...
  for (let i = 0; i < logs.length; i++) {
    ...
    ul.select("#" + log_name)
      ...
      .attr("id", log_name + '-body');

    for (let i = 0; i < attribute_list.length; i++) { 
      let attribute_name = attribute_list[i];
      let attribute_data = [];
      let single_value_attribute_list = [];
      ...
      if (log_name != "timestamp" && !allEqual(attribute_data)) {
        ...
        lineChartGen2(log_name + "-body", {
          ...
          console.log('something went wrong with', log_data, log_name);
      }
      } else if (attribute_name != "timestamp") {
        //console.log(record,vals);
        single_value_attribute_list.push(attribute_name)
      }

      let valCard = d3.select('#' + log + '-body').append('div')
      .attr('id', 'valcard' + log)
      .attr('class', 'card-panel')
      //console.log(single_value_attribute_list.length);
      for (let i = 0; i < single_value_attribute_list.length; i++) {

        let single = single_value_attribute_list[i];
    ...
    if (log != '') {
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        if (Array.isArray(log_data[0][record])){
          console.log()
          continue
        }
